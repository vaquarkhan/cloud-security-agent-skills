...
const DEFAULT_RAW_BASE_URL =
  "https://raw.githubusercontent.com/vaquarkhan/cloud-security-agent-skills/main";
...
function activate(context) {
  ...
  register("cloudSecurityAgentSkills.installFullToolkit", async () => {
    ...
    await installFiles(context, root, manifest, all, "full toolkit");
  });

  register("cloudSecurityAgentSkills.installCorePack", async () => {
    const root = getWorkspaceRoot();
    if (!root) return;
    await installFiles(context, root, manifest, dedupe([...files.core, ...files.skills]), "core pack");
  });

  register("cloudSecurityAgentSkills.installAgentAdapters", async () => {
    const root = getWorkspaceRoot();
    if (!root) return;
    const choices = [...Object.keys(files.adapters), "All"];
    const picked = await vscode.window.showQuickPick(choices, {
      placeHolder: "Choose agent adapters (Cursor, Claude, Copilot, Gemini, Kiro)"
    });
    if (!picked) return;
    const selected =
  ...
  register("cloudSecurityAgentSkills.installStarterPack", async () => {
    const root = getWorkspaceRoot();
    if (!root) return;
    const picked = await vscode.window.showQuickPick(Object.keys(files.starterPacks), {
      placeHolder: "Choose a cloud security starter pack"
    });
    if (!picked) return;
    await installFiles(context, root, manifest, files.starterPacks[picked], `${picked} starter pack`);
  });

  register("cloudSecurityAgentSkills.installMcpTemplates", async () => {
    const root = getWorkspaceRoot();
    if (!root) return;
    const choices = [...Object.keys(files.mcp), "All"];
    const picked = await vscode.window.showQuickPick(choices, {
      placeHolder: "Choose MCP templates (Bastion gateway)"
    });
    if (!picked) return;
    const selected =
      picked === "All"
  ...
  register("cloudSecurityAgentSkills.installCloudPreset", async () => {
    const root = getWorkspaceRoot();
    if (!root) return;
    const picked = await vscode.window.showQuickPick(files.presets, {
      placeHolder: "Choose a cloud security preset (AWS, Azure, GCP, OCI, IBM Bluemix, Alibaba, PCF)"
    });
    if (!picked) return;
    await installFiles(context, root, manifest, [picked], path.basename(path.dirname(picked)));
  });

  register("cloudSecurityAgentSkills.scaffoldRunnableExample", async () => {
    const root = getWorkspaceRoot();
    if (!root) return;
    const picked = await vscode.window.showQuickPick(Object.keys(files.examples), {
      placeHolder: "Scaffold a runnable example"
    });
    if (!picked) return;
...
async function loadAsset(context, manifest, relativePath) {
  ...
  const config = vscode.workspace.getConfiguration("cloudSecurityAgentSkills");
